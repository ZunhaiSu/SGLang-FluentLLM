...
class TransferInfo:
    ...
    def from_zmq(cls, msg: List[bytes]):
        # decode_prefix_len is now msg[11] (we added it as additional message part)
        decode_prefix_len = int(msg[11].decode("ascii")) if len(msg) > 11 else 0
...
class NixlKVManager(CommonKVManager):
    ...
    def receive_decode_prefix_info(self, bootstrap_room: int) -> int:
        ...
        # In nixl implementation, decode_prefix_len is handled via ZMQ messages
        # Check to stored transfer info for this room
        if bootstrap_room in self.transfer_infos:
            for transfer_info in self.transfer_infos[bootstrap_room].values():
                if hasattr(transfer_info, 'decode_prefix_len'):
